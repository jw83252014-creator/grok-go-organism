...
export default function TerrariumGraph({
  ...
  return (
    <section className="terrarium-panel pointer-events-auto absolute left-4 right-4 top-4 z-20 rounded-md p-4 sm:left-6 sm:right-auto sm:w-[420px] max-[900px]:relative max-[900px]:left-auto max-[900px]:right-auto max-[900px]:top-auto max-[900px]:order-1 max-[900px]:w-full">
      ...
      <div className="mb-4 grid grid-cols-2 gap-2">
        {cells.slice(0, 6).map(cell => (
          <div key={cell.id} className={`min-h-14 rounded border px-3 py-2 ${roleColor(cell.role)}`}>
            <div className="truncate text-sm font-medium">{cell.name}</div>
            <div className="mt-1 text-xs opacity-75">{cell.role}</div>
          </div>
        ))}
      </div>

      <div className="h-44 overflow-hidden rounded border border-cyan-400/20 bg-black/60 p-3">
        <div className="mb-2 text-xs text-cyan-100/60">metabolic pulse log</div>
          ...
          <AnimatePresence initial={false}>
            {events.slice(0, 6).map(event => (
              <motion.div
                ...
                exit={{ opacity: 0, y: 8 }}
                className="grid grid-cols-[64px_1fr] gap-2 text-xs"
              >
                <span className="text-cyan-300/60">{shortTime(event.timestamp)}</span>
                <span className="truncate text-cyan-50/85">{event.data}</span>
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
